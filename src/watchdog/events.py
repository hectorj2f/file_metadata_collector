...
class FileModifiedEvent(FileSystemEvent):
    ...
    def __repr__(self):
        print 'Event FileModifiedEvent %s' % self.src_path
        return ("<%(class_name)s: src_path=%(src_path)s>"
                ) % (dict(class_name=self.__class__.__name__,
                          src_path=self.src_path))
...
class FileAccessedEvent(FileSystemEvent):
    ...
    def __repr__(self):
        print 'Event FileAccesedEvent %s' % self.src_path
        return ("<%(class_name)s: src_path=%(src_path)s>"
                ) % (dict(class_name=self.__class__.__name__,
                          src_path=self.src_path))
...
class FileSystemEventHandler(object):
    ...
    def dispatch(self, event):
        ...
        """
        print 'FileSystemEventHandler dispatch(..)'
        print event
    ...
    def on_accessed(self, event):
        ...
        :param event:
            Event representing file access.
        :type event:
...
class LoggingEventHandler(FileSystemEventHandler):
    ...
    def on_moved(self, event):
        print 'on_moved LoggingEventHandler'
