...
## Number of files monitored by each observer
OBSERVE_MAX_CAPACITY = 75000
        ...
        try:
          ...
          if S_ISDIR(mode):
            # It's a directory, recurse into it
             counter =  count_files(str(pathname),0)
             if level < 1:
...
def count_files(top, counter):
    ...
    for f in os.listdir(top):
        ...
        try:
          ...
          if S_ISDIR(mode):
            # It's a directory, recurse into it
             counter =  count_files(pathname, counter)
          else:
             #print 'Counter %d Filename %s' % (counter, pathname)
            # It's a file, call the callback function
             counter = counter + 1
